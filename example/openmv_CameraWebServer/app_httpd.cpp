  ...
  while (true) {
    ...
    if (!fb) {
      ...
      if (fb->format != PIXFORMAT_JPEG) {
        ...
        find_blob(temp_img);
        fb->buf=temp_img->pixels; ///<- return pixel data 

        
        
 //         bool converted = fmt2bmp(temp_img->pixels, temp_img->w * temp_img->h, temp_img->w, temp_img->h, PIXFORMAT_RGB565, &buf, &buf_len);
  



        bool jpeg_converted = frame2jpg(fb, 80, &_jpg_buf, &_jpg_buf_len);////ppppppppppppppppppppppppppppppp
        esp_camera_fb_return(fb);
        fb = NULL;
        if (!jpeg_converted) {
